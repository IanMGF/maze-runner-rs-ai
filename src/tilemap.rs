...
impl TryFrom<Vec<Vec<Tile>>> for TileMap {
    ...
    fn try_from(value: Vec<Vec<Tile>>) -> Result<Self, Self::Error> {
        ...
        if value.iter().map(|vec| vec.len()).any(|size| size != first_len) {
            Err("Rows have different lengths")
        } else if value.iter().flatten().copied().filter(|e| *e == Tile::Start).count() != 1usize {
            Err("TileMap must have exactly one Start tile")
        } else if value.iter().flatten().copied().filter(|e| *e == Tile::End).count() != 1usize {
...
impl TryFrom<String> for TileMap {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        ...
        let tiles: Vec<Vec<Tile>> = chars.iter().map(|row| {
            row.iter().filter_map(|char| {
                match char {
                    '0' => Some(Tile::Empty),
                    '1' => Some(Tile::Wall),
                    '2' => Some(Tile::Start),
        ...
        }).filter(|row| !row.is_empty()).collect::<Vec<Vec<Tile>>>();
...
impl Iterator for TileMapIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let Some(tile) = self.0.get(x, y) else { unreachable!() };
